...
runANCOVAshkMORE2 <- function(object, contrasts = "all", p.adjust="BH", random.covar = TRUE, shuffle.group = TRUE, FalconOutput=TRUE, verbose=TRUE) {
  ...
  FalconInfo.model.matrix <- model.matrix(FalconDesign, data = FalconInfo)
  matching_cols <- grep(GroupOfInterest, colnames(FalconInfo.model.matrix), value = TRUE)
  colnames(FalconInfo.model.matrix)[colnames(FalconInfo.model.matrix) %in% matching_cols] <- "GROUP"
  colnames(FalconInfo.model.matrix) <- gsub("[/ -]", "_", colnames(FalconInfo.model.matrix))
  
  find_correspondence <- function(source, target) {
    sapply(source, function(x) {
      matches <- target[sapply(target, function(y) grepl(y, x, fixed = TRUE))]
  ...
  components <- colnames(FalconInfo.model.matrix)[-1]

  running.posthoc <- NULL
  running.adj.data <- NULL

  fixed_data <- FalconInfo.binary
  ...
  fit <- lmFit(Final.working.data, FalconInfo.model.matrix)
  eb <- eBayes(fit)
  ...
  for (i in seq_len(nrow(Final.working.data))) {
    ...
    perm.temp <- as.data.frame(FalconInfo.model.matrix)
    perm.temp$DATA <- as.numeric(t(Final.working.data[i, ]))
    perm.temp$GROUP<-as.factor(perm.temp$GROUP)
    
    for (col_index in seq_along(perm.temp)) {
      # Skip the newly added DATA column (assume it's the last column)
      if (col_index == ncol(perm.temp)) next
      
      # Check if the column has 2 or fewer unique values
      if (length(unique(perm.temp[[col_index]])) <= 2) {
        # Convert to factor
        perm.temp[[col_index]] <- as.factor(perm.temp[[col_index]])
      }
    }
    ...
    if (var(perm.temp$DATA) > 0) {
      ...
      if (sum(ccc.check) == 1) {
        ...
        if (deviance(c1.step) > sqrt(.Machine$double.eps)) {
          ...
          # Remove the intercept from both
          c1_final_coefs_no_intercept <- names(c1.final$coefficients[-1])  # Remove the intercept from c1.final coefficients
          mod_coefs_no_intercept <- names(mod.coefs.i[-1])         # Remove the intercept from mod.coefs.i
          shrk.coef<-mod.coefs.i[-1]
          find_best_match <- function(names, variables) {
            sapply(names, function(name) {
              matches <- variables[sapply(variables, function(var) grepl(var, name, ignore.case = TRUE))]
              if (length(matches) > 0) matches[1] else NA  # Take the first match, if any
            })
          }
          c1_original <- find_best_match(c1_final_coefs_no_intercept, variables)
          mod_original <- find_best_match(mod_coefs_no_intercept, variables)
          position <- match(c1_original, mod_original)
          c1.final$coefficients[-1]<-shrk.coef[position]
          ...
          if (length(c1.final[[1]]) > length(levels(perm.temp$GROUP))) {
            ...
            temp2 <- perm.temp[, find_correspondence(names(adj.coefficients), names(perm.temp))]
