...
runANCOVAshkMORE <- function(object, contrasts = "all", p.adjust="BH", random.covar = TRUE, shuffle.group = TRUE, FalconOutput=TRUE, verbose=TRUE) {
  ...
  FalconInfo.binary <- apply_conversion(FalconInfo.binary, diff, verbose=T)
  ...
  colnames(mod.coefs)[grep(GroupOfInterest, colnames(mod.coefs), value = F)] <- "GROUP"
    ...
    if (var(perm.temp$DATA) > 0) {
      ...
      if (sum(ccc.check) == 1) {
        ...
        if (deviance(c1.step) > sqrt(.Machine$double.eps)) {
          ...
          names(mod.coefs) <- c1.final$coefficients["GROUP"]
      
          
          c1.final$coefficients <- mod.coefs
